class PlayState{
    ...
    enter(params){
      this.shopAnim = params.shopAnim;
      this.time = 60;
      this.count();
      for (const [key, value] of Object.entries(gSounds)) {
        value.load();
      }
      gSounds.hit.play();
    }
    ...
    */
    update(){
      ...
      if(this.player.health == 0){
        ...
        this.enemy.end('win');
        gStateMachine.change('win', [this.player, this.enemy, 'Kenji', this.time]);
      }
      else if(this.enemy.health == 0){
        this.player.end('win');
        this.enemy.end('lost');
        gStateMachine.change('win', [this.player, this.enemy, 'Mack', this.time]);
      }
      else if(this.time == 0){
        this.player.end('timed');
        this.enemy.end('timed');
        gStateMachine.change('win', this.player.health>this.enemy.health?
        [this.player, this.enemy, 'Mack', this.time]:this.player.health==this.enemy.health?[this.player, this.enemy, 'Nobody', this.time]:
        [this.player, this.enemy, 'Kenji', this.time]);
      }
    }
